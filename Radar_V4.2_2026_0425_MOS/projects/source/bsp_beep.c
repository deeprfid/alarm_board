...
#define BEEP_ENABLE()	 Bsp_pwm_init()			      /* 使能蜂鸣器鸣叫 */
#define BEEP_DISABLE()	 TMRA_Stop(TMRA_UNIT)			/* 禁止蜂鸣器鸣叫 */
...
void BEEP_Stop(void)
{
    ...
    BEEP_DISABLE();
    TMRA_PWM_SetForcePolarity(TMRA_UNIT, TMRA_PWM_CH, TMRA_PWM_FORCE_LOW);
    mutex_beep_unlock();
