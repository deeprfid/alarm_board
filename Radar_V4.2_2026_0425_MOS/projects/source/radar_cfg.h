 ...
 *       裸 0x00FE 只在"重扫急救包"里、且满足长时间全静默门控时才补发, 见下面 RADAR_PROBE_FAILSAFE。
 */
...
#define RADAR_PARAM_MOVE_SENS           { 30U, 30U, 30U, 30U, 30U, 30U, 30U, 30U, 30U }
#define RADAR_PARAM_STILL_SENS          { 100U, 100U, 100U, 100U, 100U, 100U, 100U, 100U, 100U }  /* 门0/1 静止灵敏度不可设 */
 ...
 *
 * **门控(必须有)**: 急救包只在"该口**长时间一个字节都收不到**"时才发, 阈值见 RADAR_FAILSAFE_SILENT_MS。
 *   为什么: 客户手机 APP 配模块时, 模块同样会停止上报, 但 APP 的命令/ACK 一直在线上跑 ——
 *   只要线上还有字节, 门控就不放行, 我们那帧 0x00FE 就**插不进 APP 的事务里**。
 *   反过来: 卡在配置态的模块是**彻底静默**的(连 ACK 都不回), 门控必然放行 —— 该救的仍然救得到。
 *   没有门控时的现场后果: 链路失联兜底只要 3 秒静默就触发重扫, 而人的一次 APP 操作中间停顿
 *   轻松超过 3 秒, 于是急救包插进 APP 会话、把模块拉出配置态, APP 下一条命令被判"无效"。
 *
 *   0 = 关闭(不补发)
...
/* 急救包门控: 该口连续多久收不到**任何字节**才允许补发裸 0x00FE。
 * 取值依据: APP 的一次配置会话是"人操作"的节奏 —— 中间停顿几秒很常见, 但整场会话里线上一直有
 * 字节(命令/ACK)。30 秒全静默基本排除了"人的会话", 只留下真正哑掉的模块。
 * 调大更保守(更不容易撞上 APP), 调小更激进(救得更快)。 */
