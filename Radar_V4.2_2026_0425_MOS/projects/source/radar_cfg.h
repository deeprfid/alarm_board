...
/* ---- 雷达口 USART 时钟分频 / 过采样 (与波特率强相关的必配项, 两个要一起看) ----
 * 硬件: C = PCLK1(本工程 100MHz) / 分频; DDL 只支持 /1, /4, /16, /64 四档
 *       (PR.PSC 是 2 位, 实际分频 = 4^PSC, 见 hc32_ll_usart.c:581)。
 * 约束: BRR 的整数分频只有 8 位 -> DIV_Integer = C/(B*8*(2-OVER8)) - 1 必须 <= 255,
 *       即 最高档要求 C >= 8*(2-OVER8)*B_max, 最低档要求 C <= 8*(2-OVER8)*256*B_min。
 *   C = 25MHz    (DIV4) : 8 倍过采样下限 12207 -> **9600 算不出来**(SetBaudrate 返回错误
 *                         且**不写 BRR**, 端口保持复位波特率); 16 倍过采样 6103~488281, 8 档全可用。
 *   C = 6.25MHz  (DIV16): 8 倍过采样 3052~244140, 8 档全可用;
 *                         16 倍过采样上限只有 244140 -> 460800 不行。
 *   C = 1.5625MHz(DIV64): SDK 例程里 115200 / 9600 用的就是这一档 —— 那是"只有一个低波特率"
 *                         时把精度做到最好的写法(误差 0.003%), >=230400 三档全部算不出来,
 *                         我们要求 8 档全兼容, 不能照抄。
