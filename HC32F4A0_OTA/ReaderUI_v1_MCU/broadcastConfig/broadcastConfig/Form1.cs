...
namespace broadcastConfig
{
    public partial class Form1 : Form
    {
        ...
        private void btnStop_Click(object sender, EventArgs e)
        {
            ...
            using (StreamWriter writer = new StreamWriter(devPath))
            {
                ...
                foreach (ListViewItem item in lvDevs.Items)
                {
                    writer.WriteLine(item.SubItems[1].Text); // 1 表示第二列
                }
        ...
        public static void RestartProcess(string processName)
        {
            ...
            foreach (var process in Process.GetProcessesByName(processName))
            {
                ...
                {
                    process.Kill();
                    Console.WriteLine($"Process {process.ProcessName} ({process.Id}) has been terminated.");
                }
                catch (Exception ex)
                {
                    // 处理异常，比如进程访问被拒绝等  
                    Console.WriteLine($"An error occurred while trying to terminate process {process.ProcessName} ({process.Id}): {ex.Message}");
                }
            }

        //    System.Threading.Thread.Sleep(1000); // 等待1秒作为示例  

            // 启动新的进程实例  
            string appPath = AppDomain.CurrentDomain.BaseDirectory + "ReaderManager.exe";
            try
            {
