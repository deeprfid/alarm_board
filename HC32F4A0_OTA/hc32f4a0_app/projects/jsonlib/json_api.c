...
static json_value *find_value(json_value *value, const char *name)
{
    ...
    for (j = 0; j < cnt; j++)
    {
        ...
        {
            json_value *arr = cur;
            /* v9.82f: 修复 "field[idx]" 路径——isarray 段若 name 非空，须先按 name 找字段（得数组），
             * 再按 arrindex 取元素。原逻辑忽略 name、直接当 cur 是数组，根对象时必失败
             * （httpEascfg 设置 "filter_rule[0].ch_num" 因此解析失败）。纯索引 "arr[idx]" name 空则 arr=cur。 */
            if (segs[j].name[0] != '\0')
            {
                arr = NULL;
                if (cur && cur->type == json_object)
                {
                    ...
                    for (i = 0; i < (int)cur->u.object.length; i++)
                    {
                        ...
                        {
                            arr = cur->u.object.values[i].value;
                            break;
                        }
                    }
                }
            }
            if (arr && arr->type == json_array && segs[j].arrindex >= 0 &&
                segs[j].arrindex < (int)arr->u.array.length)
            {
                cur = arr->u.array.values[segs[j].arrindex];
                found = 1;
            }
        }
                       ...
                       int len, int cmpmode, char *val)
{
    ...
    /* v9.81cb: cmpmode==2 或其它 → 不检查长度（旧 json-parser 语义，PARSE_CHK_STR 标准用法）——
     * 修复 "ip" dose not find：IP "192.168.1.250"(14B) < len 29 被误拒 */
    strcpy(val, o->u.string.ptr);
    return 0;
}
