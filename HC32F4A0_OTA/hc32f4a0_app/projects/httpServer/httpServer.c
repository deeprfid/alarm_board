				...
				case STATE_HTTP_IDLE :
					if ((len = getSn_RX_RSR(s)) > 0)
					{
						...
						*(((uint8_t *)http_request) + len) = '\0';
						...
						TRACE("\r\n");
						TRACE("> HTTPSocket[%d] : HTTP Request received ", s);
						TRACE("from %d.%d.%d.%d : %d\r\n", destip[0], destip[1], destip[2], destip[3], destport);
#endif
#ifdef _HTTPSERVER_DEBUG_
						TRACE("> HTTPSocket[%d] : [State] STATE_HTTP_REQ_DONE\r\n", s);
#endif
						...
						while(getSn_TX_FSR(s) != (getSn_TxMAX(s)))
						{
							...
							{
#ifdef _HTTPSERVER_DEBUG_
								TRACE("> HTTPSocket[%d] : [State] STATE_HTTP_REQ_DONE: TX Buffer clear timeout\r\n", s);
#endif
								break;
							}
						}
...
#ifdef _HTTPSERVER_DEBUG_
					TRACE("> HTTPSocket[%d] : [State] STATE_HTTP_RES_INPROC\r\n", s);
#endif
...
#ifdef _HTTPSERVER_DEBUG_
					TRACE("> HTTPSocket[%d] : [State] STATE_HTTP_RES_DONE\r\n", s);
#endif
...
static void send_http_response_header(uint8_t s, uint8_t content_type, uint32_t body_len, uint16_t http_status)
{
	switch(http_status)
	{
		case STATUS_OK: 		// HTTP/1.1 200 OK
			...
			{
#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response Header - STATUS_OK\r\n", s);
#endif
			...
			{
#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response Header - NONE / CGI or XML\r\n", s);
#endif
...
#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response Header - STATUS_BAD_REQ\r\n", s);
#endif
			memcpy(http_response, ERROR_REQUEST_PAGE, sizeof(ERROR_REQUEST_PAGE));
			break;
		case STATUS_NOT_FOUND:	// HTTP/1.1 404 Not Found
#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response Header - STATUS_NOT_FOUND\r\n", s);
#endif
	...
	{
#ifdef _HTTPSERVER_DEBUG_
		TRACE("> HTTPSocket[%d] : [Send] HTTP Response Header [ %d ]byte\r\n", s, (uint16_t)strlen((char *)http_response));
#endif
...
#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response body - file name [ %s ]\r\n", s, HTTPSock_Status[get_seqnum].file_name);
#endif
/////////////////////////////////////////////////////////////////////////////////////////////////

#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response body - file len [ %ld ]byte\r\n", s, file_len);
#endif
		}
		else
		{
...
#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response end - file len [ %ld ]byte\r\n", s, send_len);
#endif
		}
#ifdef _USE_FLASH_
...
#ifdef _HTTPSERVER_DEBUG_
			TRACE("> HTTPSocket[%d] : HTTP Response body - send len [ %ld ]byte\r\n", s, send_len);
#endif
	}
...
/*****************************************************/
	...
	else if(HTTPSock_Status[get_seqnum].storage_type == SDCARD)
	{
		...
		if(fr != FR_OK)
		{
			send_len = 0;
#ifdef _HTTPSERVER_DEBUG_
		TRACE("> HTTPSocket[%d] : [FatFs] Error code return: %d (File Read) / HTTP Send Failed - %s\r\n", s, fr, HTTPSock_Status[get_seqnum].file_name);
#endif
		}
		else
		{
...
#ifdef _HTTPSERVER_DEBUG_
	TRACE("> HTTPSocket[%d] : [Send] HTTP Response body [ %ld ]byte\r\n", s, send_len);
#endif
	...
	else
	{
		HTTPSock_Status[get_seqnum].file_offset += send_len;
#ifdef _HTTPSERVER_DEBUG_
		TRACE("> HTTPSocket[%d] : HTTP Response body - offset [ %ld ]\r\n", s, HTTPSock_Status[get_seqnum].file_offset);
#endif
	}
...
#ifdef _HTTPSERVER_DEBUG_
	TRACE("> HTTPSocket[%d] : HTTP Response Header + Body - CGI\r\n", s);
#endif
	send_len = sprintf((char *)buf, "%s%d\r\n\r\n%s", RES_CGIHEAD_OK, file_len, http_body);
#ifdef _HTTPSERVER_DEBUG_
	TRACE("> HTTPSocket[%d] : HTTP Response Header + Body - send len [ %d ]byte\r\n", s, send_len);
#endif
...
static void http_process_handler(uint8_t s, st_http_request * p_http_request)
{
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_GET :
			...
			TRACE("> HTTPSocket[%d] : Request Type = %d\r\n", s, p_http_request->TYPE);
			TRACE("> HTTPSocket[%d] : Request URI = %s\r\n", s, uri_name);
#endif
			...
			else
			{
				...
				{
#ifdef _HTTPSERVER_DEBUG_
					TRACE("> HTTPSocket[%d] : Unknown Page Request\r\n", s);
#endif
				...
				{
#ifdef _HTTPSERVER_DEBUG_
					TRACE("> HTTPSocket[%d] : Find Content [%s] ok - Start [%ld] len [ %ld ]byte\r\n", s, uri_name, content_addr, file_len);
#endif
				...
				{
#ifdef _HTTPSERVER_DEBUG_
					TRACE("> HTTPSocket[%d] : Requested content len = [ %ld ]byte\r\n", s, file_len);
#endif
		...
		case METHOD_POST :
			...
			TRACE("> HTTPSocket[%d] : Request URI = %s ", s, uri_name);
			TRACE("Type = %d\r\n", p_http_request->TYPE);
#endif
...
uint8_t display_reg_webContent_list(void)
{
	...
	{
		TRACE(">> Web content file not found\r\n");
		ret = 0;
	}
	else
	{
		...
		{
			TRACE(" [%d] ", i+1);
			TRACE("%s, ", web_content[i].content_name);
			TRACE("%ld byte, ", web_content[i].content_len);

			if(web_content[i].content_len < 30) TRACE("[%s]\r\n", web_content[i].content);
			else TRACE("[ ... ]\r\n");
		}
		TRACE("=========================================\r\n\r\n");
		ret = 1;
