...
### 8.4 参数配置与急救包的取舍（2026-09-15 现场回归后定稿）

- **参数自动配置出厂默认关**（`RADAR_PARAM_EN = 0`）：它会跟模块做 `0x00FF -> 命令 -> 0x00FE` 事务，
  而客户会拿手机 APP 配同一个模块 —— 两边交叉下发就互相打断（现场 APP 报「设置距离门灵敏度失败 返回码 6401」）。
  要配参数时改成 `2`，逐口读数确认后改回 `0` 再出货。
- **探测期急救包（裸 `0x00FE`）保留但加门控**：只在「该口连续 ≥30 s 收不到任何字节」时才发
  （`RADAR_FAILSAFE_SILENT_MS`）。卡死模块必然全静默所以救得到；APP 会话期间线上有字节所以插不进去。
- 判读补充：`g_radar_comm` 的 **bit11..15** 只在 `RADAR_PARAM_EN != 0` 时才有参数码 ——
  默认关掉时这 5 位恒为 0，现场别把它当成故障。
- 完整数据、根因与残留风险见 `docs/radar_baud_debug_notes.md` §10（两次受控实验）与 §11（回归与门控）。

### 8.5 雷达灵敏度改由报警下行下发（2026-09-15）

现场要求：报警下行包 `alarm_pdu` 的 **`Alarm_Duration[1]`**（`common.c` 里的 `radar_range`）**每次报警带下来**，
HC32 按它设置雷达灵敏度。落地口径（已与现场确认）：

| 项 | 定案 |
| --- | --- |
| 映射 | `0` = 不设置；`1~10` -> **动态(运动)灵敏度 10~100**（值 × 10）；其它值（如流程会改成的 `0xFF`）忽略 |
| 静态阈值 | **恒为 100** |
| 写哪些门 | **0~8 全部 9 个门**（`0x0064` 是按门逐个写的） |
| 哪些口 | **本板三个雷达口都写**，哪个在线（锁定）写哪个；不在线的保持待办，上线后自动补写 |
| 幂等 | **值没变一条命令都不发**；值变了**先 `0x0061` 读回、只写不一致的门**（模块自己存 flash，所以重启后第一次下发也是先读后写） |
| 与自动配置 | `RADAR_PARAM_EN` 保持 **0**（整段关）；灵敏度**只认下行这一个来源**，两个都开会被 `#error` 拦住 |
| 阻塞 | `common.c` 只登记目标值；真正的命令在 `radar_poll()` 里**每拍只推进一个口的一个门** |
| **不在报警期间动雷达** | 参数是跟报警包一起来的，而读/写事务期间模块**停上报**（`0x00FF` 进配置态）—— 报警期间正需要雷达数据。所以每条 PDU 只记一次时刻，真正的读/写要等到**距最近一次报警下行 >= `RADAR_DL_SENS_QUIET_MS`（默认 5s）的平静期**才做；**不设强制超时**。等待期间状态码 **14** |

现场判读：`g_radar_comm` 的 **bit11..15**（`PARAM_EN=0` 时就是本功能）
= `0` 没下发过 / `1..10` **已生效档位**（灵敏度 = 值×10）/ `11` 正在写 / `12` 写失败已放弃 / `13` 该口未锁定 / `14` 在等平静期。
