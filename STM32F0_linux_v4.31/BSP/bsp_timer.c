...
uint32_t sysruntimetick=0;
void systemruntimetick(void)
{
   sysruntimetick++;

}
void bsp_STM32H750_timer_init(void)
{
	
 bsp_StartHardTimer(1,512,systemruntimetick);


}	


uint64_t  getruntimetick(void)//获取系统时间，单位ms
{
     uint32   tcount = TIM_GetCounter(TIM_HARD);
		return sysruntimetick;
}
